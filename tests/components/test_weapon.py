import unittest
from dodge.components import Position, Faction, Weapon
...
class TestWeaponComponent(unittest.TestCase):
    def setUp(self):
        ...
        self.shooter = Entity(0, 0, [Position(5, 5, self.stack),
                                     Weapon(self.stack, self.path, self.power, self.speed, self.targeting_radius)])
    ...
    def test_properties_of_projectile(self):
        target = Entity(0, 0, [Position(4, 3, self.stack), Faction(Factions.DEFENDER)])
        self.level.add_entity(target)

        self.assertIsInstance(Event, self.shooter.handle_event(self.fire_event))
    ...
    def test_fires_at_nearest_target(self):
        ...
        # TODO: Ugly
        path = self.stack.peek()[EventParam.TARGET].get_component(ComponentType.PROJECTILE).path
