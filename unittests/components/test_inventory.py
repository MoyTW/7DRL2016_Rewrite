...
from unittests.utils import EventStackStub, LevelStub
...
class TestInventory(unittest.TestCase):
    def setUp(self):
        ...
        self.handler = Entity(1, 1, [Position(self.stack, 3, 7, True)])
    ...
    def test_picks_up_happy_case(self):
        level = LevelStub(None, self.item)
    ...
    def test_drops_happy_case(self):
        ...
        level = LevelStub(None, None)
        event = Event(EventType.DROP_ITEM, {EventParam.HANDLER: self.handler,
                                            EventParam.ITEM: self.item,
                                            EventParam.LEVEL: level})
        self.inventory.handle_event(event)
        self.assertEqual(0, self.inventory.size)
        ...
        self.assertEqual(3, item_pos.x)
        self.assertEqual(7, item_pos.y)
